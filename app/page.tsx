...
// Static stats (updated periodically, or fetched from a cached API)
const STATS = {
  scholarships: '2.5K+',
  countries: '85+',
  universities: '1.2K+',
...
// Fetch only the minimal data needed for home page
async function getHomePageData() {
  await connectToDatabase();

  // Get featured scholarships (limit to 8)
  const featuredScholarships = await Scholarship.find({ featured: true, status: 'open' })
    .limit(8)
    .lean();

  // Get latest blog posts (limit to 8)
  const blogPosts = await BlogPost.find({ published: true, type: { $ne: 'guide' } })
    .sort({ createdAt: -1 })
    .limit(8)
    .lean();

  // Get top fields using aggregation (this is fast with indexes)
  ...
  // Get popular countries using aggregation
