// app/universities/UniversitiesContent.tsx
'use client';
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div
        ...
        style={{
          backgroundImage: 'linear-gradient(135deg, rgba(11,59,47,0.85), rgba(26,93,74,0.85)), url(https://images.unsplash.com/photo-1541339907198-e08756dedf3f?q=80&w=1920&auto=format)',
      ...
      >
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-white/80 text-base md:text-lg max-w-2xl mx-auto">
            Browse {STATS.universities} prestigious institutions offering fully funded scholarships.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-white rounded-xl border border-gray-200 p-6 text-center">
            ...
            <div className="text-3xl font-bold text-[#1A1A1A]">{STATS.universities}</div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 text-center">
            ...
            <div className="text-3xl font-bold text-[#1A1A1A]">{STATS.scholarships}</div>
            <p className="text-gray-500 text-sm mt-1">Total Scholarships</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-6 text-center">
            <div className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center mx-auto mb-4">
              <Globe className="w-6 h-6 text-amber-600"/>
            </div>
            <div className="text-3xl font-bold text-[#1A1A1A]">{STATS.countries}</div>
            <p className="text-gray-500 text-sm mt-1">Host Countries</p>
          </div>
        </div>

        {/* University Grid / Loading */}
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500">
              {searchQuery ? 'No matching universities found.' : 'No institutions found.'}
            </p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentUniversities.map(u => (
                <div key={u.name} className="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all hover:-translate-y-1">
                  <CardImage src={u.image || ''} alt={u.name} fallbackText={u.name} height="h-40"/>
                  <div className="p-5">
                    <h3 className="font-serif text-lg font-semibold text-[#1A1A1A] group-hover:text-[#0B3B2F] transition-colors mb-2">
                      {u.name}
                    </h3>
                    <div className="flex items-center gap-2 text-sm text-gray-500 mb-3">
                      <MapPin className="w-4 h-4 flex-shrink-0"/>
                      <span className="line-clamp-1">
                        {u.countries.slice(0,3).join(', ')}
                        {u.countries.length > 3 && ` +${u.countries.length-3} more`}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">
                        {u.count} scholarship{u.count !== 1 ? 's' : ''}
                      </span>
                      <Link
                        href={`/scholarships?search=${encodeURIComponent(u.name)}`}
                        className="inline-flex items-center gap-1 text-sm font-medium text-[#0B3B2F] hover:text-[#D4A373] transition-colors"
                      >
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-2 mt-10">
                <button
                  onClick={() => updateUrl(Math.max(1, currentPage - 1))}
                ...
                <button
                  onClick={() => updateUrl(Math.min(totalPages, currentPage + 1))}
            ...
            <div className="mt-12 text-center">
              ...
              >
                Browse All Scholarships <ExternalLink className="w-4 h-4"/>
              </Link>
            </div>
          </>
        )}
