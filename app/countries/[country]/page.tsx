  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-white">
        <div className="relative text-white py-20 md:py-24" style={{ backgroundImage: `linear-gradient(135deg, rgba(11, 59, 47, 0.8), rgba(26, 93, 74, 0.8)), url(${heroImageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: '#0B3B2F' }}>
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <h1 className="font-serif text-3xl md:text-5xl lg:text-6xl font-bold mb-3">{decodedCountry}</h1>
                <p className="text-white/80 text-base md:text-lg max-w-2xl">Discover fully funded scholarships and study opportunities in {decodedCountry}.</p>
              </div>
              <div className="flex gap-6 text-center">
                <div><div className="text-3xl md:text-4xl font-bold">{totalCount}</div><div className="text-white/70 text-sm">Total Scholarships</div></div>
              </div>
            </div>
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              {scholarships.length === 0 ? (
                <div className="bg-gray-50 rounded-2xl border border-gray-200 p-12 text-center">
                  <div className="w-16 h-16 bg-[#0B3B2F]/10 rounded-full flex items-center justify-center mx-auto mb-4"><MapPin className="w-8 h-8 text-[#0B3B2F]" /></div>
                  <h2 className="text-xl font-serif text-[#1A1A1A] mb-2">No scholarships found</h2>
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">We couldn't find any scholarships for {decodedCountry}. Check back later or browse other countries.</p>
                  <Link href="/scholarships" className="inline-flex items-center gap-2 bg-[#0B3B2F] text-white px-5 py-2.5 rounded-lg font-medium hover:bg-[#1A5D4A] transition-colors">Browse All Scholarships</Link>
                </div>
              ) : (
                <>
                  <div className="flex items-center justify-between mb-6"><h2 className="font-serif text-2xl text-[#1A1A1A]">Available Scholarships ({scholarships.length})</h2></div>
                  <div className="space-y-3">
                    {scholarships.map((scholarship: any) => {
                      const deadlineHint = getDeadlineHint(new Date(scholarship.deadline));
                      const fundingString = Array.isArray(scholarship.fundingType) ? scholarship.fundingType[0] : scholarship.fundingType || 'Fully Funded';
                      return (
                        <Link key={scholarship._id} href={`/scholarships/${scholarship.slug}`} className="block">
                          <div className="group bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-all hover:border-[#D4A373]">
                            <div className="flex items-start gap-4">
                              <div className="w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden"><CardImage src={scholarship.image || ''} alt={scholarship.title} fallbackText={scholarship.provider} height="h-16" /></div>
                              <div className="flex-1 min-w-0">
                                <h3 className="font-medium text-sm text-[#1A1A1A] group-hover:text-[#0B3B2F] transition-colors line-clamp-2 mb-1">{scholarship.title}</h3>
                                <p className="text-gray-500 text-xs mb-2">{scholarship.provider}</p>
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
                                  <span className="flex items-center gap-1"><GraduationCap className="w-3 h-3" />{Array.isArray(scholarship.degreeLevel) ? scholarship.degreeLevel[0] : scholarship.degreeLevel}</span>
                                  <span className="flex items-center gap-1"><Award className="w-3 h-3" />{fundingString}</span>
                                  <span className="flex items-center gap-1 text-[#0B3B2F] font-medium"><Clock className="w-3 h-3" />{deadlineHint}</span>
                                </div>
                              </div>
                            </div>
                          </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-gray-50 rounded-xl border border-gray-200 p-6"><h3 className="font-serif text-lg font-semibold mb-4 flex items-center gap-2"><Globe className="w-5 h-5 text-[#0B3B2F]" />Quick Stats</h3><div className="grid grid-cols-2 gap-4"><div className="bg-white rounded-lg p-3 text-center"><GraduationCap className="w-5 h-5 text-[#0B3B2F] mx-auto mb-1" /><div className="text-xl font-bold text-[#1A1A1A]">{degreeStats.length}</div><div className="text-xs text-gray-500">Degree Types</div></div><div className="bg-white rounded-lg p-3 text-center"><Award className="w-5 h-5 text-[#0B3B2F] mx-auto mb-1" /><div className="text-xl font-bold text-[#1A1A1A]">{fundingStats.length}</div><div className="text-xs text-gray-500">Funding Types</div></div><div className="bg-white rounded-lg p-3 text-center col-span-2"><Users className="w-5 h-5 text-[#0B3B2F] mx-auto mb-1" /><div className="text-xl font-bold text-[#1A1A1A]">{totalCount}</div><div className="text-xs text-gray-500">Total Opportunities</div></div></div></div>
              {degreeStats.length > 0 && (<div className="bg-white rounded-xl border border-gray-200 p-6"><h3 className="font-serif text-lg font-semibold mb-4">Degree Levels</h3><div className="space-y-3">{degreeStats.map((stat: any) => (<div key={stat._id} className="flex items-center justify-between"><span className="text-gray-600">{stat._id}</span><span className="font-medium bg-gray-100 px-2 py-1 rounded-full text-sm">{stat.count}</span></div>))}</div></div>)}
              {fundingStats.length > 0 && (<div className="bg-white rounded-xl border border-gray-200 p-6"><h3 className="font-serif text-lg font-semibold mb-4">Funding Types</h3><div className="space-y-3">{fundingStats.map((stat: any) => (<div key={stat._id} className="flex items-center justify-between"><span className="text-gray-600">{stat._id}</span><span className="font-medium bg-gray-100 px-2 py-1 rounded-full text-sm">{stat.count}</span></div>))}</div></div>)}
              <div className="bg-gradient-to-br from-[#0B3B2F] to-[#1A5D4A] rounded-xl p-6 text-white"><h3 className="font-serif text-lg font-semibold mb-2">Ready to apply?</h3><p className="text-white/80 text-sm mb-4">Browse all scholarships and find your perfect opportunity.</p><Link href="/scholarships" className="inline-block w-full text-center bg-[#D4A373] text-[#0B3B2F] px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-[#C67B5E] transition-colors">Browse All Scholarships</Link></div>
            </div>
          </div>
        </div>
      </main>
