...
export default function GuidesPage() {
  ...
  useEffect(() => {
    const fetchGuides = async () => {
      try {
        ...
        const guidePosts = (data.posts || []).filter((p: any) => p.type === 'guide');
  ...
  const filteredGuides = useMemo(() => {
    ...
    return guides.filter(guide => {
      const searchableText = `${guide.title} ${guide.excerpt} ${guide.category} ${guide.author}`.toLowerCase();
  ...
  const heroImageUrl = 'https://images.unsplash.com/photo-1434030216411-0b793f4b4173?q=80&w=1920&auto=format';
      ...
      <main className="min-h-screen bg-gray-50">
        ...
        >
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="font-serif text-4xl md:text-5xl font-bold mb-4">Scholarship Guides</h1>
            <p className="text-white/80 text-lg max-w-2xl mx-auto">
              In-depth resources to help you navigate every step of the scholarship application process.
            </p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-8">
            ...
            {searchQuery && (
              <p className="text-sm text-gray-500 mt-3">
                Found {filteredGuides.length} result{filteredGuides.length !== 1 ? 's' : ''}
              </p>
            )}
          </div>
            ...
            <>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentGuides.map((guide) => (
                  <Link key={guide._id} href={`/guides/${guide.slug}`} className="block h-full">
                    <div className="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all hover:-translate-y-1 duration-300 h-full flex flex-col">
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        ...
                        <p className="text-gray-500 text-sm line-clamp-3 mb-4">{guide.excerpt}</p>
                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                          <span className="text-sm text-gray-600">{guide.author}</span>
                          <span className="text-[#0B3B2F] text-sm font-medium group-hover:text-[#D4A373] transition-colors inline-flex items-center gap-1">
