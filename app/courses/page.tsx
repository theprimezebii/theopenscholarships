...
export default function CoursesPage() {
  ...
  useEffect(() => { if (!mounted) return; fetchCourses(); }, [fetchCourses, mounted]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border p-4 mb-6">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={() => updateUrl({ search: searchInput || null, page: '1' })} className="bg-[#0B3B2F] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#1A5D4A] transition-colors flex items-center justify-center gap-2">
                <Search className="w-4 h-4" /> Search
              </button>
            </div>

            {/* Desktop Filters – single line (wraps naturally but no clipping) */}
            <div className="hidden lg:block mt-4">
              ...
              {totalFiltersApplied > 0 && (
                <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-100">
                  ...
                  {selectedLevel && (
                    <span className="bg-amber-50 text-amber-700 text-xs px-2 py-1 rounded-full flex items-center gap-1">{selectedLevel} <button onClick={() => updateUrl({ level: null })} className="hover:text-red-500">×</button></span>
                  )}
                  {certificateFilter && (
                    <span className="bg-teal-50 text-teal-700 text-xs px-2 py-1 rounded-full flex items-center gap-1">{certificateFilter === 'true' ? 'With Certificate' : 'Without Certificate'} <button onClick={() => updateUrl({ certificate: null })} className="hover:text-red-500">×</button></span>
                  )}
                  {ratingFilter && (
                    <span className="bg-pink-50 text-pink-700 text-xs px-2 py-1 rounded-full flex items-center gap-1">≥ {ratingFilter} ☆ <button onClick={() => updateUrl({ rating: null })} className="hover:text-red-500">×</button></span>
                  )}
                  {searchQuery && (
                    <span className="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded-full flex items-center gap-1">"{searchQuery}" <button onClick={() => updateUrl({ search: null })} className="hover:text-red-500">×</button></span>
                  )}
                </div>
              )}
            </div>
          ...
          {loading ? (
            ...
            <>
              ...
              {totalPages > 1 && (
                <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={(page) => updateUrl({ page: String(page) })} />
              )}
            </>
          )}
        </div>

        {/* Sticky Filter Button – centered, auto width, moderate rounding */}
