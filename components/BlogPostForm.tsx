...
export default function BlogPostForm({ initialData, onSubmit, submitLabel }: BlogPostFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    faqs: (initialData?.faqs || []).map(f => ({ question: f.question, answer: f.answer })), // strip _id
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (data.success) {
        const previewPath = formData.type === 'guide' 
          ? `/guides/${data.slug}` 
          : `/blog/${data.slug}`;
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Guides appear in /guides with a full‑width hero; articles appear in /blog.
            </p>
          </div>
          <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="mt-4">
          ...
          <p className="text-xs text-gray-500 mt-1">This image will be used as the hero background on the detail page.</p>
        </div>
      </div>
